...
| 2026-07-09 | Phase 0    | 進入整理工作台前加入密碼輸入           | 建議在 `App.tsx` 統一攔截頁籤與原始資訊卡片入口，使用課堂示範密碼 `pika`；不使用真實密碼或正式權限控管             | 採用        | 這符合使用者要求，但前端密碼只能作為課堂門檻，不能視為安全機制                              | `src/app/App.tsx`, `src/features/phase-0/Phase0RawInfoPanel.tsx`, `src/styles/global.css`, `tests/app-smoke.test.tsx`, `tests/phase0-workbench.test.tsx`                                 |
...
| 2026-07-09 | Phase 0    | 新增繁中 / 英文語言切換                | 建議建立前端語言 context，讓首頁、原始資訊、整理工作台、狀態與來源標籤可切換繁中與英文，並用 localStorage 保留選擇 | 採用        | 這只切換介面文案，不翻譯或改寫原始資訊內容，也不改變查核狀態或整理判斷                      | `src/i18n/language.tsx`, `src/app/App.tsx`, `src/components/**`, `src/features/phase-0/**`, `tests/app-smoke.test.tsx`                                                                   |
| 2026-07-09 | Phase 0    | 英文模式下翻譯原始資訊原文             | 建議新增前端用的 Phase 0 原文翻譯對照表，英文模式顯示翻譯文字，詳細彈窗保留原始中文供追溯                          | 採用        | 採用顯示層翻譯；不修改 fixture 原始資料、不呼叫外部翻譯或 LLM API，也不把翻譯視為已確認事實 | `src/i18n/phase0-content.ts`, `src/components/RecordCard.tsx`, `src/features/phase-0/Phase0RawInfoPanel.tsx`, `src/features/phase-0/Phase0JudgementCard.tsx`, `tests/app-smoke.test.tsx` |
| 2026-07-09 | Phase 0    | 將主背景改為多層動態背景               | 建議用 CSS 線性光帶與細格線製作緩慢漂移的背景，明暗模式各自調整色彩，並尊重 reduced-motion 設定                    | 採用        | 這只改善視覺層次，不改變原始資訊、查核狀態、資料流程或任何救災判斷                          | `src/styles/global.css`, `docs/ai-log.md`                                                                                                                                                |
| 2026-07-09 | Phase 0    | 原始資訊區塊加入小彩蛋                 | 建議在原始資訊標題列放入低調的小按鈕，連續點三下後顯示指定圖片與「被找到了！」文字                                 | 採用        | 這只是前端視覺彩蛋，不會修改原始資訊、查核狀態、整理工作台或任何資料流程                    | `IMG_20260709_173431.png`, `src/features/phase-0/Phase0RawInfoPanel.tsx`, `src/styles/global.css`, `tests/phase0-workbench.test.tsx`                                                     |
...
| 2026-07-09 | Phase 0    | 詳細彈窗顯示新增原始資訊完整欄位       | 建議把新增表單取得的來源、時間、地點或範圍、確認方式、備註與缺漏狀態保存在該筆原始資訊，點開詳細資料時完整呈現     | 採用        | 這只補足欄位透明度；仍維持待人工確認，不把新增資料顯示成已確認                              | `src/features/phase-0/phase0-types.ts`, `src/features/phase-0/Phase0RawInfoPanel.tsx`, `src/styles/global.css`, `tests/phase0-workbench.test.tsx`                                        |
| 2026-07-09 | Phase 0    | 點擊詳細彈窗外部可關閉                 | 建議讓詳細資訊彈窗的背景遮罩負責關閉，彈窗內容本身阻止冒泡，避免查看欄位時誤關閉                                   | 採用        | 這只改善檢視互動，不改資料內容、查核狀態或整理判斷                                          | `src/features/phase-0/Phase0RawInfoPanel.tsx`, `tests/phase0-workbench.test.tsx`                                                                                                         |
| 2026-07-09 | Phase 0    | 詳細彈窗加入複製摘要互動               | 建議在詳細資訊彈窗加入一鍵複製摘要，將既有原文、來源、查核狀態與新增欄位整理成可貼到討論中的文字                   | 採用        | 這是前端檢視輔助，只複製畫面既有資訊，不修改原始資料、查核狀態或整理判斷                    | `src/components/source-labels.ts`, `src/components/SourceLabel.tsx`, `src/features/phase-0/Phase0RawInfoPanel.tsx`, `src/styles/global.css`, `tests/phase0-workbench.test.tsx`           |
| 2026-07-09 | Release 02 | 用 Mermaid 繪製資訊整理流程圖          | 建議把 v1 流程整理成自然語言與 Mermaid，明確標示 AI 只產生建議、人工確認與不能自動處理分支                         | 採用        | 符合流程設計階段要求；本次只更新流程文件，不修改前端程式                                    | `release-packs/02-flow-design-kit/docs/flow.md`, `docs/ai-log.md`                                                                                                                        |
| 2026-07-09 | Release 02 | 使用 02-flow-design-kit 確認流程圖     | 依 design-checklist 檢查流程是否對應資訊整理者、保留原始資訊、不讓 AI 成為決策者，並補上流程確認結果               | 採用        | 修正主要使用者焦點，確認流程能支撐後續實作；仍需人類在 VS Code 預覽 Mermaid                 | `release-packs/02-flow-design-kit/docs/flow.md`, `docs/ai-log.md`                                                                                                                        |
| 2026-07-09 | Release 03 | 依 flow.md 實作不完整資訊收錄流程      | 建議只要求原始資訊內容必填，缺來源、時間、地點或確認方式時仍收入清單，並標示不完整、待人工確認與判斷紀錄           | 採用        | 符合流程圖「資訊不足仍可收入清單」；不把缺漏資料標示為已確認，也不自動變成任務              | `src/features/phase-0/Phase0RawInfoPanel.tsx`, `src/features/phase-0/phase0-heuristics.ts`, `tests/phase0-workbench.test.tsx`, `tests/phase0-heuristics.test.ts`                         |
